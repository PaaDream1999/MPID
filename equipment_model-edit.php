...
<div id="box-form">
<h2 id="title-txt">แก้ไขข้อมูลครุภัณฑ์</h2> 
...
<?php
		  		if(!empty($_GET['ID'])){
					$sql = mysqli_query($con,"Select * From equipment_model Where id_equipment_model='".$_GET['ID']."'");
					$rs = mysqli_fetch_array($sql);
					include 'table-sql/tb-equipment_model.php'; 
					}
		  ?>

<table border="0" cellspacing="1" style="width: 100%; margin-bottom: 5px;">
  <tr>
    <th width="19%" align="right">ประเภทครุณฑ์  </th>
    <td width="81%">
	<select  name="type_id" id="select" style="width: 200px; padding:5px;">
      ...
      <?php
								...
								while($rs=mysqli_fetch_array($sql)){
												...
												if($type_id==$rs['id_equipment_type']){
  ...
  <tr>
    ...
    <td><input  name="name" type="text" id="txt" value="<?=$name?>" /></td>
  </tr>
  <tr>
    <th align="right">รายละเอียด</th>
    <td><textarea rows="10" id="txt" style="width: 75%;" name="detail" ><?=strip_tags($detail)?></textarea></td>
  </tr>
    <tr>
    <th align="right">ราคา  </th>
    <td><input  name="price" type="text" id="txt" maxlength="5" value="<?=$price?>"/></td>
  </tr>
    <tr>
    <th align="right">หมายเหตุ</th>
    <td><textarea rows="3" id="note" style="width: 75%;" name="note" ><?=strip_tags($note)?></textarea></td>
  </tr>
    <tr>
      <th align="right">&nbsp;</th>
      <td>
