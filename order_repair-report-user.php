...
$TitleKeyword = $_POST['keyword'];
  if(!empty($TitleKeyword)){
	...
	$q="SELECT * FROM $tableSql WHERE(code LIKE '%".$keyword."%') $showStatus and member_id='".$_SESSION['sess_mb_id']."' ORDER BY id_$tableSql Desc";  
	}else{

	$q="SELECT * FROM $tableSql Where $showStatus and member_id='".$_SESSION['sess_mb_id']."' ORDER BY id_$tableSql Desc";  
