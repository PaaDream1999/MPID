...
<div id="box-list">		
<h2 id="title-list">ข้อมูลปัญหาและวิธีการซ่อมครุภัณฑ์ </h2> 
<?php  
$TitleKeyword = $_POST['keyword'];
  if(!empty($TitleKeyword)){
	...
	$q="SELECT * FROM $tableSql WHERE(title LIKE '%".$keyword."%')  ORDER BY id_$tableSql Desc";  
	}else{
...
<?php  //	ถ้าไม่มีข้อมูล
	  ...
	  <table id="table-hover" width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-condensed  table-bordered" style="font-size: 12px;">
        <tr>
          ...
          <td id="f-center" width="55%" align="center" valign="middle" bgcolor="#F4F4F4"><strong>ปัญหาอาการเสียของครุภัณฑ์</strong></td>
          <td id="f-center" width="20%" align="center" valign="middle" bgcolor="#F4F4F4"><strong>ประเภทครุภัณฑ์</strong></td>
          <td id="f-center" width="7%" align="center" valign="middle" bgcolor="#F4F4F4" ><strong><i class="icon-edit"></i> แก้ไข</strong></td>
          <td id="f-center" width="7%" align="center" valign="middle" bgcolor="#F4F4F4"><strong><i class="icon-trash"></i> ลบ</strong></td>
        </tr>
...
?>
         ...
         <tr class="list-hover">
		  ...
		  <a href="main.php?page=<?=$pageUrl?>-detail&ID=<?=$ID?>"><?=$name?></a>		  </td>
          <td align="center" valign="middle"><?=$type_name?></td>
          <td id="f-center" width="7%" align="center" valign="middle">
		 <a  href="main.php?page=<?=$pageUrl?>-edit&ID=<?=$ID?>"><strong>  แก้ไข</strong></a>		 </td>
          <td id="f-center" width="7%" align="center" valign="middle"> 
		  <a   href="<?php echo $Action; ?>?Table=<?=$pageUrl?>&Sql=Delete&ID=<?=$ID?>" onclick="return confirm('ยืนยันลบข้อมูล <?=$name?> ออกจากระบบ')"><strong>  ลบ</strong></a>		  </td>
        </tr>
